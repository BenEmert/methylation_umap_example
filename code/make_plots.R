...
plot_umap_gg <- function(df, x_col="UMAP1", y_col="UMAP2", color_col="Color_dx",
  ...
  if (!is.null(label_col)) {
    ...
    # If any Label appears more than once, it means it has >1 color associated with it.
...
# Ensure metadata ID column matches sample names in data
...
# 3. Random State Comparison
# ------------------------------------------------------------------------------
message("   [PLOTTING] UMAP Random State Comparison...")
emb_list <- list() # Initialize list for dataframes
plot_list <- list() # Initialize list for plots
...
for (s in c(0, 1)) {
  ...
  # Store dataframe for the Zoom plots later
...
p_zoom0 <- plot_umap_gg(dat0, xlim=c(0, 7), ylim=c(-4.6, 4.6))
ggsave(file.path(plot_dir, "umap_GPL13534_M_rng0_zoom1.pdf"), p_zoom0, width=7, height=7)

p_box0 <- plot_umap_gg(dat0, rect_coords=list(xmin=-0.5, xmax=7.5, ymin=-5.1, ymax=5.1))
...
p_zoom1 <- plot_umap_gg(dat1, xlim=c(1, 8), ylim=c(-4.6, 4.8))
ggsave(file.path(plot_dir, "umap_GPL13534_M_rng1_zoom1.pdf"), p_zoom1, width=7, height=7)

p_box1 <- plot_umap_gg(dat1, rect_coords=list(xmin=0.5, xmax=8.5, ymin=-5.1, ymax=5.3))
